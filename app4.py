...
def run_python_code(python_code: str, dependencies: list = None, retry: bool = False):
    """
...
"""
    ...
    try:
        ...
        logger.error(f"Python code execution failed: {e.stderr}")
        ...
        if not retry:
            ...
            {uv_script}
            Error: {e.stderr}
            Please regenerate the code to handle the error. Do not give anything else , just give the code , do not say anything , just the code
            """

            retry_response = call_llm("You are an AI that generates Python code for tasks.", retry_prompt)
            retry_code = retry_response["choices"][0]["message"]["content"]
            logger.info(f"Regenerated Python code: {retry_code}")
            return run_python_code(retry_code, dependencies, retry=True)
    ...
    try:
        ...
        functions = [
            ...
            },
            {
                ...
                "description": "Run a script by downloading it from a URL and executing it with uv.",
                "parameters": {
                    "type": "object",
                    "properties": {
        ...
        if function_name == "get_input_files":
            ...
            code_functions = [
                {
                    ...
                    "description": "Run a Python task by generating and executing Python code. Assume docker environment. You can also use subprocesses to execute shell commands in the docker like npx run etc for task involving embedding use text 3 embedding small . For similarity also use text 3 embedding small to vectorise using transformers library . Even if no dependency is required always give uv as a dependency",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "python_code": {"type": "string", "description": "Python code to execute."},
                            "dependencies": {"type": "array", "items": {"type": "string"}, "description": "List of Python dependencies to install. Do not use their popular name,use the name by which i can install directly from pip. Like for sklearn use scikit-learn"}
                        },
            ...
            return run_python_code(python_code, dependencies)
